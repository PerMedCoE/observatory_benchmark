...
import re
def create_parser():
    parser = argparse.ArgumentParser(description="Perform analysis of Biodynamo Diffusion of 1000k cells as sink")
    
    parser.add_argument("--data_folder", action="store", dest = "data_folder",help="folder were the output data is stored",default="results/")
    parser.add_argument("--csvout", action="store", dest="csv_fname", default="concentration_all.csv",
...
def create_gif(output_folder,csv_fname):
    ...
    for index, row in df.iterrows():
        if index % 10 == 0:
            ...
            ax.set_xlabel('x')
            ax.set_ylabel('y')
            plt.title("BioDynaMo Diffusion of molecules, 1k cells as sinks Timestep: " + f'{index:.2f}')
            plt.savefig(os.path.join(output_folder, f"diff_timepoint{int(index)}.png"))
            plt.close()
