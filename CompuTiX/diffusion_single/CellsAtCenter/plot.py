...
def main() -> None:
    ...
    #Compute analytical solution - concentration at central position
    ...
    for fname in files:
        ...
        if(not args.without_center):
            ...
            rel_error = abs( c_central[-1] - c_expected_central[-1] ) / abs( c_expected_central[-1] )
            plt.figure( 100 )
    ...
    if(not args.without_center):
        ...
        plt.figure( 10 )
        plt.plot( time_steps / 60., 1e3 * c_expected_central, label='Expected', color='black' )
        plt.xlabel( 't [min]' )
        plt.ylabel( 'c [µM]' )
        ax = plt.gca()
        ax.xaxis.set_major_formatter(ticker.ScalarFormatter(useOffset=False))
        ...
        #Plot at stationary state - relative error at central position
        plt.figure( 100 )
        plt.xlabel( 'N [1]' )
        ...
        fig.tight_layout()
        fig.set_size_inches(8, 6)
        plt.savefig( f'{prefix}center_of_box_error_stationary_state.png', bbox_inches='tight', dpi = 600 )
        plt.close()
    ...
    if(not args.without_stationary_2D):
        ...
        plt.figure( 300 )
        plt.pcolormesh(1e6 * x_analytical, 1e6 * y_analytical, 1e3 * c_2D_expected, shading='auto', cmap='viridis')
        plt.xlabel( 'x [µm]' )
        plt.ylabel( 'y [µm]' )
        ax = plt.gca()
        ax.xaxis.set_major_formatter(ticker.ScalarFormatter(useOffset=False))
        ...
        fig.tight_layout()
        fig.set_size_inches(8, 6)
        plt.savefig( f'{prefix}stationnary_state_analytical.png', bbox_inches='tight', dpi = 600 )
        plt.close()
