...
def main() -> None:
    ...
    #Compute equilibrium contact radius
    a_eq = np.cbrt((3*np.pi*E*r**2*(2*gamma - tau)) / (32*(1 - nu**2))) #[m]
    print( f'Equilibrium contact radius: {a_eq:.2e} m' )
    ...
    #Plot equilibrium distance
    d_eq = 2*r - (2*a_eq**2)/r #[m]
    plt.axhline( d_eq/1e-6, color='blue', linestyle='--', label='equilibrium distance' )
    print( f'Equilibrium distance: {d_eq} m' )
