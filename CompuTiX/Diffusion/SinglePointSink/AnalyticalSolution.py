        ...
        for i in range( 0, self.count_modes ):
            for j in range( 0, self.count_modes ):
                for k in range( 0, self.count_modes ):
                    ...
                    #Sink part
                    sol -= self.rate * v_ijk * self.eigenfunction( i=i, j=j, k=k, x=self.x_cell ) * ( 1. - np.exp( -lambda_ijk * t ) ) / lambda_ijk
                    #Diffusion part
                    sol -= self.c_0 * A_ijk * v_ijk * np.exp( -lambda_ijk * t )
        ...
        for i in range( 0, self.count_modes ) :
            for j in range( 0, self.count_modes ) :
                for k in range( 0, self.count_modes ) :
                    ...
                    #Sink part
                    sol -= self.rate * v_ijk_int * self.eigenfunction( i=i, j=j, k=k, x=self.x_cell ) * ( 1. - np.exp( -lambda_ijk * t ) ) / lambda_ijk
                    #Diffusion part
                    sol -= self.c_0 * A_ijk * v_ijk_int * np.exp( -lambda_ijk * t )
