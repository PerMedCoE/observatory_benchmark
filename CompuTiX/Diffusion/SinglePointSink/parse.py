...
def parse_data( input_dir: Path ) -> dict :
    ...
    for n, f in enumerate( files ):
        ...
        if( U[n] < 0 ).any() :
            print( f'Warning: negative value at time {t[n]} [s]: U = {U[n]} [mol] in file {f}' )
    ...
    return { "D" : { "description" : "Diffusion coefficient", "unit" : universe["D"].unit, "values" : universe["D"].values[0].tolist() },\
             ...
             "x_cell" : { "description" : "Cells' positions", "unit" : universe["Cells/x"].unit, "values" : universe["Cells/x"].values[0].tolist() },\
