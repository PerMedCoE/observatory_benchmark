//! \example Diffusion/BoxInitiallyFull/CellsAtCenter/main.cpp
//! \section diffusion_box_initially_full_cells_at_center Diffusion on a box initially full with single point sink
//!
//! \subsection diffusion_box_initially_full_cells_at_center_problem_statement Problem statement
//!
//! This simulation illustrates diffusion of a species in a box with multiple point sinks
//! representing cells with a constant uptake rate.
//! The equation to solve is then a reaction-diffusion equation with a homogeneous source term
//! \f{align*}
//! \partial_t c(\vec{x},t) - \nabla \cdot \left( D \nabla c(\vec{x},t) \right) &=
//! -\sum \limits _{l} \lambda_l \delta_{\vec{x}_l}( \vec{x} )  &\text{if}& \; \vec{x} \in \left(-\frac{L}{2}, \frac{L}{2} \right)^3
//! \\ c(\vec{x}) &= c_\text{BC} \;
...
//! \f$D\f$ the diffusion coefficient and \f$\lambda_l\f$ the uptake rate of the cell \f$l\f$,
//! \f$\delta_{\vec{x}_l}\f$ the Dirac's function at the cell's position \f$\vec{x}_l\f$
...
//!
//! \subsection diffusion_box_initially_full_cells_at_center_analytical solution Analytical solution
//!
//! The analytical solution is given as
//! \f{align*}
//!     c(\vec{x},t) &= c_\text{BC} - \sum \limits _{l} \lambda_l \sum \limits _{ijk}^\infty \phi_{ijk}(\vec{x}) \phi_{ijk}(\vec{x}_l) \frac{1 - \mathrm{e}^{-\lambda_{ijk}t}}{\lambda_{ijk}}
...
//! \f}
//!
//! \subsection diffusion_box_initially_full_cells_at_center_finite_volume Finite volume scheme
//!
...
//! \f[
//! \partial_t \int \limits_{V_\alpha} \mathrm{d}V \; c = \int \limits_{V_\alpha} \mathrm{d}V \; D \nabla^2 c - \int \limits_{V_\alpha}\mathrm{d}V
//! \sum \limits _{l:\vec{x}_l \in V_\alpha} \lambda_l \delta_{\vec{x}_l}( \vec{x} )
//! \f]
//! which gives considering \f$c_\alpha = \frac{n_\alpha}{V_\alpha}\f$, Gauss's theorem and integration of dirac functions,
//! \f[
//! \partial_t n_\alpha  = \oint \limits_{\partial V_\alpha} \mathrm{d}\vec{A} \cdot D \nabla c - \sum \limits _{l:\vec{x}_l \in V_\alpha} \lambda_l.
//! \f]
//! Then,
//! \f{align*}
//!    \partial_t n_\alpha &= \sum \limits_{\beta \in \text{Neigh}(\alpha)} A_{\alpha\beta}D
//!    \frac{ c_\beta - c_\alpha }{d_{\alpha\beta}} - \sum \limits _{l:\vec{x}_l \in V_\alpha} \lambda_l
//! \\ &= \frac{D}{a^2}\sum \limits_{\beta \in \text{Neigh}(\alpha)}
//!    \left( n_\beta - n_\alpha\right) - \sum \limits _{l:\vec{x}_l \in V_\alpha} \lambda_l
//! \f}
//! where the first sum is over the neighbouring voxels of \f$\alpha\f$ and the second one is over all cells that belong to voxel \f$\alpha\f$
...
//! = n_\alpha^t - \Delta t \; \sum \limits _{l:\vec{x}_l \in V_\alpha} \lambda_l
//! \f]
...
int main( int argc, char** argv )
{
    ...
    //Add options
    {
        ...
        option_adder( "lambda", "Individual cell's uptake rate [mol/s].", cxxopts::value< Types::Scalar >()->default_value( "2.666666666666666e-18" ) );
    ...
    // - IO
    constexpr auto output_interval = 6.; //[1]
    ...
    //Create cells
    {
        //Add cell and set up its position at center of domain
        auto view = Particles::make_particles_view( Access::Modes::read_write, cells );

        std::array< Types::Position, 12 > positions = { Types::Position{ -10e-6, -10e-6, 0. },
                                                        Types::Position{ -10e-6, 10e-6, 0. },
                                                        Types::Position{ -10e-6, -30e-6, 0. },
                                                        Types::Position{ -10e-6, 30e-6, 0. },
                                                        Types::Position{ 10e-6, -10e-6, 0. },
                                                        Types::Position{ 10e-6, 10e-6, 0. },
                                                        Types::Position{ 10e-6, -30e-6, 0. },
                                                        Types::Position{ 10e-6, 30e-6, 0. },
                                                        Types::Position{ -30e-6, -10e-6, 0. },
                                                        Types::Position{ -30e-6, 10e-6, 0. },
                                                        Types::Position{ 30e-6, -10e-6, 0. },
                                                        Types::Position{ 30e-6, 10e-6, 0. } };
        for( auto x: positions ) {
            ...
            //Set position and bounding boxes to be exactly the cell itself - cell is a point
            cell.set< Types::Position >( Access::Modes::read_write, "x", SIUnits::meter, x );
            cell.set< Types::Position >( Access::Modes::read_write, "AABB_min", SIUnits::meter, x + ( -epsilon * Types::Vector::Ones() ) );
            cell.set< Types::Position >( Access::Modes::read_write, "AABB_max", SIUnits::meter, x + epsilon * Types::Vector::Ones() );
        }
    }
