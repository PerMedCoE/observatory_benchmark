...
def parse_data( input_dir: Path ) -> dict :
    ...
    for f in files:
        ...
        x.append(float(spheres["x"].values[0][0])) #[m]
        v.append(float(spheres["v"].values[0][0])) #[m/s]
    ...
    return {
        ...
        "x": {
            "description": "Positions of a first cell over time x(t)",
            "unit": spheres["x"].unit,
            "values": x
        },
        "v": {
            "description": "Velocities of a first cell over time v(t)",
            "unit": spheres["v"].unit,
            "values": v
        },
    }
